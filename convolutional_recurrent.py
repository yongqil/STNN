...
class ConvGRU2DCell(Layer):
     ...
     """
        ...
        def call(self, inputs, states, training=None):
            ...
            x_z = self.input_conv(inputs, self.kernel_z, self.bias_z,
                                  padding=self.padding)
            x_r = self.input_conv(inputs, self.kernel_r, self.bias_r,
                                  padding=self.padding)
            x_h = self.input_conv(inputs, self.kernel_h, self.bias_h,
                                  padding=self.padding)
            h_z = self.recurrent_conv(h_tm1,
                                      self.recurrent_kernel_z)
            h_r = self.recurrent_conv(h_tm1,
                                      self.recurrent_kernel_r)
            h_h = self.recurrent_conv(h_tm1,
                                      self.recurrent_kernel_h)
            if 0 < self.dropout < 1 and self._dropout_mask is None:
                self._dropout_mask = _generate_dropout_mask(
                    K.ones_like(x_z),
            ...
            if 0 < self.dropout < 1.:
                x_z = x_z * dp_mask[0]/(1-self.dropout)
                x_r = x_r * dp_mask[1]/(1-self.dropout)
                x_h = x_h * dp_mask[2]/(1-self.dropout)

            if 0 < self.recurrent_dropout < 1.:
                h_z = h_z * rec_dp_mask[0]/(1-self.recurrent_dropout)
                h_r = h_r * rec_dp_mask[1]/(1-self.recurrent_dropout)
                h_h = h_h * rec_dp_mask[2]/(1-self.recurrent_dropout)

            z = self.recurrent_activation(x_z + h_z)
            r = self.recurrent_activation(x_r + h_r)
            h_1 = self.activation(x_h + r * h_h)
            h = (1 - z) * h_1 + z * h_tm1
...
class DeConvGRU2DCell(Layer):
    """Cell class for the DeConvGRU2D layer.
 ...
 """
    ...
    def __init__(self, filters,
                 ...
                 recurrent_dropout=0.,
                 **kwargs):
        super(DeConvGRU2DCell, self).__init__(**kwargs)
        self.filters = filters
        self.kernel_size = conv_utils.normalize_tuple(kernel_size, 2, 'kernel_size')
        self.strides = conv_utils.normalize_tuple(strides, 2, 'strides')
        self.padding = conv_utils.normalize_padding(padding)
    ...
    def call(self, inputs, states, training=None):
        ...
        if inputs is not None:
            ...
            z = self.recurrent_activation( h_z)
            r = self.recurrent_activation( h_r)
            h_1 = self.activation( r * h_h)
        h = (1 - z) * h_1 + z * h_tm1
    ...
    def input_conv(self, x, w, b=None, padding='valid'):
        ...
        conv_out = K.conv2d_transpose(x, w, strides=self.strides,
                            padding=padding,
                            data_format=self.data_format,
                            dilation_rate=self.dilation_rate,
                                      output_shape=output_shape)
    ...
    def get_config(self):
        config = {'filters': self.filters,
                  ...
                  'dropout': self.dropout,
                  'recurrent_dropout': self.recurrent_dropout}
        base_config = super(DeConvGRU2DCell, self).get_config()
        return dict(list(base_config.items()) + list(config.items()))


class DeConvGRU2D(ConvRNN2D):
    """Convolutional GRU.
 ...
 """
    ...
    def __init__(self, filters,
                 ...
                 recurrent_dropout=0.,
                 **kwargs):
        cell = DeConvGRU2DCell(filters=filters,
                             kernel_size=kernel_size,
                             strides=strides,
                             padding=padding,
                             data_format=data_format,
                             ...
                             dropout=dropout,
                             recurrent_dropout=recurrent_dropout)
        super(DeConvGRU2D, self).__init__(cell,
                                        return_sequences=return_sequences,
                                        go_backwards=go_backwards,
                                        stateful=stateful,
                                        **kwargs)
    ...
    def call(self, inputs, mask=None, training=None, initial_state=None):
        return super(DeConvGRU2D, self).call(inputs,
                                           mask=mask,
                                           training=training,
                                           initial_state=initial_state)
    ...
    def get_config(self):
        config = {'filters': self.filters,
                  ...
                  'dropout': self.dropout,
                  'recurrent_dropout': self.recurrent_dropout}
        base_config = super(DeConvGRU2D, self).get_config()
        del base_config['cell']
        return dict(list(base_config.items()) + list(config.items()))
